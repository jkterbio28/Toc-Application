...
//        log.info("[preHandle][" + request + "]" + "[" + request.getMethod() + "]" + request.getRequestURI()); // + getParameters(request));
        String toKafka = "[preHandle][" + request + "]" + "[" + request.getMethod() + "]" + request.getRequestURI(); // + getParameters(request));
//        producer.sendMessage(toKafka);
//        producer.sendMessage("test");
    ...
    public void afterCompletion(final HttpServletRequest request, final HttpServletResponse response, final Object handler, final Exception ex) throws Exception {
        if (ex != null)
            ex.printStackTrace();
        log.info("[afterCompletion][" + request + "][exception: " + ex + "][Response: " + response.getClass() + "]");
    }
